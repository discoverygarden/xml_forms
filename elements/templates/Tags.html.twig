  ...
  <span class="tag-list">
    <?php foreach ($tags as $tag): ?>
      <span title="<?php print "{$tag['#value']}" ?>">
        <span class="edit-tag" onclick="jQuery('#<?php print $edit[$tag['#hash']]['#id'] ?>').trigger('mousedown'); return false;"><?php print \Drupal\Component\Utility\Html::escape("{$tag['#value']}") ?></span>
